...
async function fetchHtmlWithFallback(url, storeName) { 
  ...
  if (storeName === 'Flipkart') {
      ...
      runLayer1 = false;
      runLayer2 = true;  // Only Microlink
      runLayer3 = false; // Scrape.do Blocked for Flipkart Testing
      runLayer4 = false;
  } else if (storeName === 'Myntra' || storeName === 'Ajio') {
      console.log("🎯 STRICT ROUTING: Myntra/Ajio -> Direct to Layer 3 (Scrape.do) to save time!");
      runLayer1 = false; 
      runLayer2 = false; 
      runLayer3 = true;  // Direct Hit
      runLayer4 = false; // Focus strictly on Scrape.do
  }
  ...
  // 🔵 LAYER 2: MICROLINK (Timeout: 15s) - 🔥 FIX APPLIED HERE 🔥
  if (runLayer2 && !finalHtml) {
      try {
          console.log("➡️ Trying Layer 2 (Microlink)...");
          // 🚨 Timeout 5000ms se badha kar 15000ms (15 seconds) kar diya hai
  ...
  // 🟣 LAYER 3: MULTI-TOKEN SCRAPE.DO (Timeout: 20s)
