...
async function fetchHtmlWithFallback(url) {
  ...
  if (!finalHtml) {
      try {
          ...
          const resMicro = await fetchWithTimeout(`https://api.microlink.io?url=${encodeURIComponent(url)}`, {}, 5000); // 👈 5 seconds
  ...
  // 🟣 LAYER 3: SCRAPE.DO (The Heavy Lifter - Timeout: 20s)
  if (!finalHtml) {
      const scrapeDoToken = process.env.SCRAPER_DO_TOKEN?.replace(/"/g, '').trim();
      if (scrapeDoToken) {
          try {
              console.log("➡️ Trying Layer 3 (Scrape.do)...");
              // 🚨 Scrape.do ko ab poore 20 seconds milenge apna magic dikhane ke liye!
              const res2 = await fetchWithTimeout(`http://api.scrape.do?token=${scrapeDoToken}&url=${encodeURIComponent(url)}`, {}, 20000); // 👈 20 seconds
              if (res2.ok) {
  ...
  if (!finalHtml) {
      try {
          ...
          const res3 = await fetchWithTimeout(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`, {}, 5000); // 👈 5 seconds
