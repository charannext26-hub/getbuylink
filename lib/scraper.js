...
export async function getOgTags(targetUrl) {
  try {
    ...
    let title = '';
    let image = '';
    // 🚨 FIX: Default price ab khali string hai
    let price = ""; 
    ...
    if (storeName === 'Amazon') {
      ...
      if (asin) {
        ...
        // 🚨 CLEAN AMAZON UI FIX: Agar lamba kachra text aaye, toh khali kar do
    ...
    if (!title || title.includes("Access Denied") || title.includes("Just a moment") || title.includes("Robot Check") || title.includes("Are you a human")) {
