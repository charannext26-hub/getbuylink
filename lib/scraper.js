...
// ASIN nikalne wala function pehle upar rakh diya hai taaki niche use ho sake
export function extractASIN(url) {
  const asinMatch = url.match(/\/(?:dp|gp\/product|exec\/obidos\/ASIN|o)\/([A-Z0-9]{10})/i);
  return asinMatch ? asinMatch[1] : null;
}

// 1. THE DEEP EXPANDER MACHINE (WITH AMAZON BYPASS)
export async function expandUrl(shortUrl, depth = 0) {
  ...
  try {
    ...
    // Direct return kar do taaki Amazon Vercel ko ban na kare.
    ...
    // 🚨 NAYA LOGIC: Amazon aur Meesho short links ko manually handle karenge taaki Website load na ho.
    const isManualRedirect = shortUrl.includes('meesho.com/b') || shortUrl.includes('amzn.to') || shortUrl.includes('amzn.in/');
    ...
    // Agar server ne redirect (301/302) diya, toh naya link header se pakdo (Bina page load kiye)
