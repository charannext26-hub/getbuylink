...
async function fetchHtmlWithFallback(url) {
  ...
  const isBlocked = (htmlOrTitle, isHtml = true) => {
      ...
      if (isHtml) {
          ...
          // Fake Title Rejector (Agar Microlink kachra title de toh block karo)
  ...
  // 🟢 LAYER 1: DIRECT VERCEL FETCH (Timeout: 2.5s)
  try {
  ...
  // 🔵 LAYER 2: MICROLINK (Timeout: 2.5s)
  if (!finalHtml) {
      try {
          console.log("➡️ Trying Layer 2 (Microlink)...");
          const resMicro = await fetchWithTimeout(`https://api.microlink.io?url=${encodeURIComponent(url)}`, {}, 2500);
          ...
          if (resMicro.status !== 429) {
              ...
              if (microData && microData.status === "success" && microData.data) {
                  ...
                  // 🚨 BULLSHIT DETECTOR: Agar title generic hai, toh reject karo!
                  if (extractedTitle && !isBlocked(extractedTitle, false)) {
                      ...
                      console.log("⚠️ Layer 2 Gave Fake/Generic Data. Rejecting & Moving to Layer 3!");
                  }
              }
          } else console.log("⚠️ Layer 2 Rate Limited.");
      } catch(e) { console.log("⚠️ Layer 2 Failed/Timeout"); }
  }

  // 🟣 LAYER 3: SCRAPE.DO (The Heavy Lifter - Timeout: 5s)
  if (!finalHtml) {
      const scrapeDoToken = process.env.SCRAPER_DO_TOKEN?.replace(/"/g, '').trim();
      if (scrapeDoToken) {
          try {
              console.log("➡️ Trying Layer 3 (Scrape.do)...");
              const res2 = await fetchWithTimeout(`http://api.scrape.do?token=${scrapeDoToken}&url=${encodeURIComponent(url)}`, {}, 15000);
              if (res2.ok) {
      ...
      } else console.log("⚠️ Layer 3 Skipped: Token missing.");
  }

  // 🟠 LAYER 4: ALLORIGINS PROXY (Timeout: 2.5s)
  if (!finalHtml) {
      try {
          console.log("➡️ Trying Layer 4 (AllOrigins)...");
          const res3 = await fetchWithTimeout(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`, {}, 2500);
