...
// ==========================================
// 🚀 THE 3-LAYER FALLBACK FETCHER
// ==========================================
async function fetchHtmlWith3LayerFallback(url) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 6000); // 6s max for all layers

  try {
    // LAYER 1: Normal Fetch (Direct Vercel)
    ...
    if (normalRes.status !== 403 && normalRes.status !== 503) {
      return await normalRes.text();
    }
    
    console.log(`⚠️ Layer 1 (Vercel) Blocked for ${url}. Switching to Layer 2...`);
    throw new Error("Blocked by Store");
  } catch (error) {
    
    // LAYER 2: Microlink (Daily 50 Free)
    try {
        console.log("➡️ Trying Layer 2 (Microlink)...");
        const microRes = await fetch(`https://api.microlink.io?url=${encodeURIComponent(url)}&prerender=true&meta=false`);
        if (microRes.status !== 429) {
            const microData = await microRes.json();
            if(microData && microData.data && microData.data.html) return microData.data.html;
        } else {
            console.log("⚠️ Layer 2 (Microlink) Rate Limited (429). Switching to Layer 3...");
            throw new Error("Microlink Rate Limited");
        }
    } catch (microErr) {
        
        // LAYER 3: Scrape.do (Monthly 1,000 Free)
        try {
            console.log("➡️ Trying Layer 3 (Scrape.do)...");
            const scrapeDoToken = process.env.SCRAPER_DO_TOKEN;
            if (scrapeDoToken) {
                const scrapeDoRes = await fetch(`http://api.scrape.do?token=${scrapeDoToken}&url=${encodeURIComponent(url)}`);
                if(scrapeDoRes.ok) return await scrapeDoRes.text();
            }
        } catch (scrapeDoErr) {
            console.log("⚠️ Layer 3 (Scrape.do) Failed.");
        }
    }
    
    // LAYER 4: The Ultimate Survival (AllOrigins Proxy)
    console.log("➡️ Trying Layer 4 (AllOrigins Proxy)...");
    try {
      const proxyRes = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`);
      const proxyData = await proxyRes.json();
      if (proxyData && proxyData.contents) return proxyData.contents;
    } catch (proxyErr) {
...
export async function getOgTags(targetUrl) {
  try {
    ...
    let title = '';
    let image = '';
    let price = "Best Price"; // Default Clean Price
    ...
    if (storeName === 'Amazon') {
      ...
      if (asin) {
        ...
        // 🚨 CLEAN AMAZON UI FIX: Agar lamba kachra text aaye, toh "Best Price" kar do
        let safePrice = amazonData.price || "Best Price";
        if (safePrice.length > 20 || safePrice.toLowerCase().includes("check price")) {
            safePrice = "Best Price";
        }
    ...
    if (storeName === 'Meesho') {
        if (fallbackImage && fallbackImage.includes('http')) {
            return { success: true, originalUrl: targetUrl, expandedUrl: finalUrl, title: fallbackTitle || "Meesho Exclusive Deal", description: "", image: fallbackImage, store: storeName, price: "Best Price" };
        }
    ...
    // --- STORE 3: FLIPKART, MYNTRA & OTHERS (With 3-Layer Proxy Bypass) ---
    if (storeName !== 'Amazon' && !image) {
        try {
          // 🚨 NAYA: 3-Layer Fetcher ko bulaya
          const html = await fetchHtmlWith3LayerFallback(finalUrl);
          ...
          if (html) {
            ...
            if (storeName === 'Flipkart') {
               ...
               // Flipkart Price Extractor (Bonus Feature)
    ...
    return { success: false, title: "Exclusive Deal", description: "", image: "", price: "Best Price" };
  }
}