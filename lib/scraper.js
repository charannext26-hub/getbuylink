...
async function fetchHtmlWithFallback(url, storeName) { 
  ...
  if (runLayer4 && !finalHtml) {
      try {
          ...
          const res3 = await fetchWithTimeout(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`, {}, 10000); 
...
export async function getOgTags(targetUrl) {
  try {
    ...
    // --- STORE 1: AMAZON (STRICTLY USE API, DO NOT RELY ON HTML FALLBACK) ---
    if (storeName === 'Amazon') {
      const asin = extractASIN(finalUrl) || extractASIN(targetUrl); // Ensure ASIN is grabbed
      if (asin) {
        ...
        // Return whatever the PAAPI got us (even if price is empty, we still use its title/image)
