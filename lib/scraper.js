...
async function fetchHtmlWithFallback(url, storeName) { 
  ...
  if (storeName === 'Flipkart') {
      ...
      runLayer4 = false; // AllOrigins usually fails on Flipkart
...
// 1. THE DEEP EXPANDER MACHINE (🚀 VERCEL SHORT LINK FIX)
export async function expandUrl(shortUrl, depth = 0) {
  if (depth > 5) return shortUrl; // Prevent infinite loops
  ...
  try {
    ...
    let response;
    try {
      // 🚨 THE MASTER FIX: ALWAYS use 'manual' redirect. 
      // Vercel server ab hawa mein hi Location header chheen lega Flipkart ke block karne se pehle!
    ...
    // STEP 1: Catch HTTP Redirects (301, 302, 308) - Short URLs usually do this
    ...
    // STEP 2: Vercel Blocked / Error (403, 503)
    ...
    // STEP 3: Status 200 (Success) - Check inside HTML for hidden JS redirects (Wishlink, etc.)
    ...
    // Agar sab attempts ke baad lamba URL nahi mila, toh wahi return kardo
    return response.url || shortUrl;

  } catch (error) { 
      // ULTIMATE FALLBACK: Agar sab fail ho jaye
      try {
