...
export async function getOgTags(targetUrl) {
  try {
    ...
    // --- STORE 1: AMAZON (PA-API WITH MICROLINK FALLBACK) ---
    if (storeName === 'Amazon') {
      ...
      if (asin) {
        try {
          ...
          if (amazonData.success && (amazonData.title || amazonData.image)) {
              console.log("✅ Amazon PA-API Success!");
              return {
              ...
              console.log("⚠️ PA-API returned empty data. Falling back to Microlink...");
          }
        } catch (apiErr) {
          console.log("⚠️ Amazon PA-API Crashed/Timeout:", apiErr.message);
          console.log("🔄 Switching to Microlink Fallback for Amazon...");
        }
      } else {
          console.log("⚠️ ASIN not found in URL (Likely a shortlink). Routing directly to Microlink Fallback...");
      }
    }

    // --- STORE 2: FLIPKART, MYNTRA & OTHERS (HTML ENGINE) ---
    try {
      ...
      if (html) {
        ...
        // 🟢 FLIPKART 100% ACCURATE JSON-LD LOGIC
        if (storeName === 'Flipkart') {
           ...
           // Search hidden JSON-LD schema for real price
           ...
           // Basic fallback agar JSON-LD na mile
           if (!price) {
        ...
        // 🟢 UNIVERSAL JSON-LD LOGIC (For Myntra, Ajio, etc)
        $('script[type="application/ld+json"]').each((i, el) => {
          try {
            const data = JSON.parse($(el).html());
