...
// 1. ASIN EXTRACTOR (Upar rakha hai taaki expandUrl isko use kar sake)
...
export async function expandUrl(shortUrl, depth = 0) {
  // 🚨 Vercel timeout bachane ke liye depth 3 par rok diya
  if (depth > 3) return shortUrl; 

  try {
    console.log(`Deep Expand Level ${depth}:`, shortUrl);

    // 🚨 VERCEL AMAZON BYPASS: Agar URL mein pehle se ASIN hai, toh usko fetch karke block mat khao!
    if ((shortUrl.includes('amazon.') || shortUrl.includes('amzn.')) && extractASIN(shortUrl)) {
        console.log("✅ Amazon ASIN found directly, skipping fetch to prevent Vercel block.");
        return shortUrl; 
    }
    ...
    // 🚨 VERCEL FIX 1: Add Timeout (Taaki 10s cross na ho aur Vercel API crash na kare)
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3500); // Max 3.5 seconds per fetch
    ...
    try {
      ...
      clearTimeout(timeoutId);
      throw fetchErr; // Agar timeout hua toh niche catch block me jayega
    }

    // 🚨 VERCEL FIX 2: Agar Amazon ne Vercel IP ko block kiya (403/503), toh Bypass API use karo
    if (response.status === 403 || response.status === 503 || response.status === 400) {
      console.log("⛔ Store Blocked Vercel IP! Using Unshorten API fallback...");
      try {
        const apiRes = await fetch(`https://unshorten.me/json/${encodeURIComponent(shortUrl)}`);
        const apiData = await apiRes.json();
        if (apiData && apiData.resolved_url) {
    ...
    // Redirect Header Capture (Bina page load kiye naya link pakadna)
    ...
    // Intermediate site brute force (cutt.ly, bit.ly, wishlink, ltl.sh)
    ...
    console.log("⚠️ Expand Error/Timeout:", error.message);
    
    // 🚨 VERCEL FIX 3: Timeout hone par aakhiri koshish (Bypass API)
    try {
        if(shortUrl.includes('amzn.to') || shortUrl.includes('amzn.in') || shortUrl.includes('fkrt.it')) {
            const apiRes = await fetch(`https://unshorten.me/json/${encodeURIComponent(shortUrl)}`);
            const apiData = await apiRes.json();
...
export async function getOgTags(targetUrl) {
  try {
    ...
    if (finalUrl.includes("meesho.com")) {
        ...
        // Agar Fallback na mile tab manual logic
    ...
    if (!finalUrl.includes("meesho.com") || (finalUrl.includes("meesho.com") && !image)) {
        try {
          const controller = new AbortController();
          const timeoutId = setTimeout(() => controller.abort(), 4000); // 4 sec timeout for HTML load
