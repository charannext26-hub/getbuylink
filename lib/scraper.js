...
async function fetchHtmlWithFallback(url, storeName) { 
  ...
  if (storeName === 'Amazon') {
      console.log("🛑 COST SAVER: Amazon -> Scrape.do BLOCKED to save credits! Using Layer 1 & 2 for basic OG tags.");
      runLayer1 = false;  
      runLayer2 = true;  
      runLayer3 = false; // ❌ DO NOT WASTE SCRAPE.DO CREDITS ON AMAZON
      runLayer4 = false; 
  } else if (storeName === 'Flipkart') {
      console.log("🎯 SMART ROUTING: Flipkart -> Try Layer 2, Fallback to Layer 3 (Scrape.do)");
      runLayer1 = false; // Flipkart blocks standard fetches heavily
      runLayer2 = true;  // Try Microlink first
      runLayer3 = true;  // 🚀 ALLOW LAYER 3 (Scrape.do) FALLBACK
      runLayer4 = false; 
  ...
  // 🟢 LAYER 1: DIRECT VERCEL FETCH (Timeout: 8s)
...
export async function expandUrl(shortUrl, depth = 0) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (location && !location.includes('play.google.com') && !location.includes('apple.com')) {
    ...
    if (response.status === 403 || response.status === 503 || response.status === 400) {
      try {
        ...
        if (apiData && apiData.resolved_url && apiData.resolved_url !== shortUrl) {
            return expandUrl(apiData.resolved_url, depth + 1);
        }
      } catch (e) { }
    }
    ...
    if (redirectJsonMatch && redirectJsonMatch[1] && redirectJsonMatch[1].startsWith('http')) {
        return expandUrl(redirectJsonMatch[1].replace(/\\u0026/g, '&').replace(/\\/g, ''), depth + 1);
    }
    
    const metaRefreshMatch = html.match(/url=(https?:\/\/[^"'>]+)/i);
    if (metaRefreshMatch && metaRefreshMatch[1]) {
        return expandUrl(metaRefreshMatch[1], depth + 1);
    }
    
    const hiddenEcomLink = html.match(/(https?:\/\/(?:www\.)?(?:flipkart\.com|dl\.flipkart\.com|amazon\.in|myntra\.com|ajio\.com|meesho\.com)[^"'\s\\]+)/i);
    if (hiddenEcomLink && hiddenEcomLink[1]) {
        return expandUrl(hiddenEcomLink[1], depth + 1);
    }
    ...
    return response.url || shortUrl;
      ...
      try {
          const apiRes = await fetchWithTimeout(`https://unshorten.me/json/${encodeURIComponent(shortUrl)}`, {}, 2000);
          const apiData = await apiRes.json();
          if (apiData && apiData.resolved_url && apiData.resolved_url !== shortUrl) return apiData.resolved_url;
      } catch(e) {}
      return shortUrl; 
  }
}
...
export async function getOgTags(targetUrl) {
  try {
    ...
    // 🎯 CRITICAL FIX: URL expand karne se pehle hi check karo ki kya ye Amazon link hai
    const initialStoreCheck = getStoreName(targetUrl);

    if (initialStoreCheck === 'Amazon') {
      console.log("🎯 Direct Amazon Route Triggered! Sending original link straight to Microlink...");
      try {
        // Hum original targetUrl (short link) ko bina Vercel se touch kiye direct Microlink bhej rahe hain
        const resMicro = await fetchWithTimeout(`https://api.microlink.io?url=${encodeURIComponent(targetUrl)}`, {
            headers: { 'User-Agent': 'Mozilla/5.0' }
        }, 12000); 
        ...
        if (resMicro.ok) {
            ...
            if (microData && microData.status === "success" && microData.data) {
                ...
                if (extractedTitle && !extractedTitle.toLowerCase().includes("access denied") && !extractedTitle.toLowerCase().includes("just a moment")) {
                    console.log("✅ Microlink Direct Amazon Success:", extractedTitle);
                    return {
                      success: true,
                      originalUrl: targetUrl,
                      expandedUrl: microData.data.url || targetUrl,
                      store: 'Amazon',
                      title: extractedTitle.replace(/\|?\s*Amazon\s*/i, '').trim(),
                      description: "",
        ...
        console.log("⚠️ Direct Microlink failed for Amazon, moving to PA-API fallback...");
      } catch (microErr) {
        console.log("⚠️ Direct Microlink Error for Amazon:", microErr.message);
      }
    }

    // -----------------------------------------------------------------
    // Baki saare stores (Flipkart, Meesho, etc.) ya Amazon fallback ke liye normal flow
    // -----------------------------------------------------------------
    ...
    // --- AMAZON PA-API FALLBACK (Agar Microlink fail ho jaye tabhi ye chalega) ---
    if (storeName === 'Amazon') {
      const asin = extractASIN(finalUrl) || extractASIN(targetUrl); 
      if (asin) {
        try {
          console.log(`➡️ Trying Amazon PA-API for ASIN: ${asin}`);
          const amazonData = await fetchAmazonProductData(asin, finalUrl);
          
          let safePrice = amazonData.price || "";
          if (safePrice.length > 20 || safePrice.toLowerCase().includes("check") || safePrice.toLowerCase().includes("best")) {
              safePrice = "";
          }
          
          if (amazonData.success && (amazonData.title || amazonData.image)) {
              return {
                success: true, originalUrl: targetUrl, expandedUrl: finalUrl, store: storeName,
                title: amazonData.title || fallbackTitle || "Amazon Product", description: "", 
                image: amazonData.image || fallbackImage || "", 
                price: safePrice, discountPercent: amazonData.discountPercent || ""
              };
          }
        } catch (apiErr) {
          console.log("⚠️ Amazon PA-API Failed:", apiErr.message);
        }
      }
    }

    // --- STORE 2: FLIPKART, MYNTRA & OTHERS (Aapka purana HTML Engine) ---
    try {
      ...
      if (html) {
        ...
        if (storeName === 'Flipkart') {
           ...
           if (!title) title = $('title').text().replace(/Buy .*?Online at Best Prices.*/i, '').trim();
               ...
               try {
                   ...
                   items.forEach(item => {
                       if (item['@type'] === 'Product' || item['@type'] === 'AggregateOffer') {
                           ...
                           else if (item.offers && item.offers.lowPrice) price = `₹${item.offers.lowPrice}`;
                       }
                   });
               } catch (e) {}
           });
           ...
           if (!price) {
               let extractedPrice = $('div._30jeq3._16Jk6d').text().trim() || $('div.Nx9-qj').text().trim();
        ...
        $('script[type="application/ld+json"]').each((i, el) => {
          try {
            ...
            if (item && (item['@type'] === 'Product' || item.name)) {
               ...
               if (!price && item.offers && item.offers.price && !String(item.offers.price).includes('₹')) {
      ...
      console.log("⚠️ HTML Fetch Failed...");
    }
    ...
    return {
      success: true, originalUrl: targetUrl, expandedUrl: finalUrl, store: storeName,
      title: title.replace(/\s+/g, ' ').trim(), description: "", image: image, price: price, discountPercent: discountPercent 
    };
