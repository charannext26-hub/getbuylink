...
// ⏱️ FAST TIMEOUT ENGINE (Vercel ke 10s kill se bachne ke liye)
...
// ==========================================
// 🚀 THE BULLETPROOF & COST-SAVING 4-LAYER FETCHER
// ==========================================
async function fetchHtmlWithFallback(url) {
  let finalHtml = null;
  const storeName = getStoreName(url); // Store check karne ke liye

  // 🛑 COST SAVER: MEESHO BLOCK
  if (storeName === 'Meesho') {
      console.log("🛑 Meesho Scrape Blocked to save API Credits. Returning generic deal.");
      return null; // Return null so fallback logic triggers generic title
  }
  ...
  // 🔵 LAYER 2: MICROLINK (Timeout: 5s)
  if (!finalHtml) {
      try {
          console.log("➡️ Trying Layer 2 (Microlink)...");
          const resMicro = await fetchWithTimeout(`https://api.microlink.io?url=${encodeURIComponent(url)}`, {}, 5000); 
          ...
          } else console.log("⚠️ Layer 2 Rate Limited or Failed.");
      } catch(e) { console.log("⚠️ Layer 2 Failed/Timeout"); }
  }

  // 🟣 LAYER 3: MULTI-TOKEN SCRAPE.DO (The Heavy Lifter - Timeout: 20s)
  if (!finalHtml) {
      // ✅ INFINITE CREDIT HACK: Multiple Tokens Check Karega
      const tokens = [
          process.env.SCRAPER_DO_TOKEN,
          process.env.SCRAPER_DO_TOKEN_2,
          process.env.SCRAPER_DO_TOKEN_3
      ].filter(t => t); // Jo token exist karta hai usko lega
      
      // Randomly ek token choose karega load balance karne ke liye
      const scrapeDoToken = tokens.length > 0 ? tokens[Math.floor(Math.random() * tokens.length)]?.replace(/"/g, '').trim() : null;

      if (scrapeDoToken) {
          try {
              console.log("➡️ Trying Layer 3 (Scrape.do) with Token...");
              // 🔥 COST SAVER FLAG: render=false (Browser band kar diya)
              let apiUrl = `http://api.scrape.do?token=${scrapeDoToken}&url=${encodeURIComponent(url)}`;
              
              const res2 = await fetchWithTimeout(apiUrl, {}, 20000); 
              if (res2.ok) {
          ...
          } catch(e) { console.log("⚠️ Layer 3 Failed/Timeout"); }
      } else console.log("⚠️ Layer 3 Skipped: No Tokens found.");
  }
...
export async function getOgTags(targetUrl) {
  try {
    ...
    try {
        const fbResponse = await fetchWithTimeout(targetUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } }, 2000);
    ...
    let image = '';
    let price = ""; 
    let discountPercent = ""; // 🚨 NAYA: Discount ka variable
    ...
    // --- STORE 2: FLIPKART, MYNTRA, MEESHO & OTHERS (With 4-Layer System) ---
    try {
    ...
    return {
      ...
      image: image,
      price: price,
      discountPercent: discountPercent // 🔥 Added output
    };
