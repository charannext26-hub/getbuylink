...
async function fetchHtmlWithFallback(url) {
  ...
  // 🔵 LAYER 2: MICROLINK (Smart JSON Fetcher)
  if (!finalHtml) {
      try {
          console.log("➡️ Trying Layer 2 (Microlink)...");
          // Ab hum HTML nahi, direct saaf metadata (JSON) maang rahe hain
          const resMicro = await fetch(`https://api.microlink.io?url=${encodeURIComponent(url)}`);
          if (resMicro.status !== 429) {
              const microData = await resMicro.json();
              if (microData && microData.status === "success" && microData.data) {
                  console.log("✅ Layer 2 (Microlink) JSON Success!");
                  
                  // Seedha JSON se data utha liya!
                  const extractedTitle = microData.data.title || "";
                  const extractedImage = microData.data.image ? microData.data.image.url : "";
                  
                  // Agar data mil gaya, toh hum ek artificial HTML bana kar aage bhej denge 
                  // taaki hamara baaki ka code aaram se chal sake.
                  if (extractedTitle && !isBlocked(extractedTitle)) {
