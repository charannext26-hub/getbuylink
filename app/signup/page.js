...
export default function SignupPage() {
  ...
  const [successMsg, setSuccessMsg] = useState(false); // Ye track karega ki email gaya ya nahi
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Yahan aapke backend ka API call hoga jo verification email bhejega
      ...
      if (!data.success) {
        ...
        // Success hone par form hide ho jayega aur ye message dikhega
        setSuccessMsg(true);
        setLoading(false);
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 font-sans p-4 sm:p-8">
      ...
      <div className="w-full max-w-[400px] bg-white p-8 rounded-[2rem] shadow-xl border border-slate-200/60">
        ...
        {/* 🚨 SUCCESS STATE: Agar email chala gaya, toh form hide ho jayega aur ye dikhega */}
        {successMsg ? (
