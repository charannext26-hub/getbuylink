...
// 🚨 Left-Right Ordering Trick for True Masonry
...
export default function CreatorBioPage({ params }) {
  ...
  const [activeTab, setActiveTab] = useState("home"); // home, trending, telegram, categories
  ...
  // 🚨 NAYA: Live Fake Sale state
  const [liveSale, setLiveSale] = useState(null); 
  
  const bannerScrollRef = useRef(null);

  useEffect(() => {
    async function fetchCreatorAndDeals() {
      try {
    ...
    if (username) fetchCreatorAndDeals();
  }, [username]);
  ...
  // 🚨 NAYA: Master Trick - Fake Live Sales Popups Generator (With Toggle Check)
  useEffect(() => {
      // Agar creator nahi hai, YA FIR usne DB se salesBoosterActive OFF rakha hai, toh kuch mat karo
      ...
      const triggerRandomSale = () => {
          ...
          setLiveSale({ store, time, amount });
          
          // Hide after 4 seconds
          setTimeout(() => setLiveSale(null), 4000);
      };

      // First sale popup after 6 seconds of landing
      const initialTimer = setTimeout(() => {
          triggerRandomSale();
          
          // Then continue randomly between 10 to 18 seconds
          const interval = setInterval(() => {
              triggerRandomSale();
  ...
  }, [creator]); // 🚨 FIX: Yahan bracket '}' add ho gaya hai
  ...
  // Helper for rendering feed items
  const renderFeedItem = (item, uniqueKey) => {
      if (item.type === 'single') {
          return <div key={uniqueKey} className="break-inside-avoid"><GridProductCard deal={item.deal} onClick={() => handleDealClick(item.deal)} themeCardClass={currentTheme.card} onToast={triggerToast} /></div>;
      } 
          ...
          return (
              <div key={uniqueKey} onClick={() => setTheatreMode({ isOpen: true, videoUrl: item.videoUrl, relatedDeals: item.deals })} className={`break-inside-avoid relative aspect-[4/5] rounded-2xl overflow-hidden cursor-pointer shadow-sm border group ${currentTheme.card}`}>
                  ...
                  <div className="absolute top-2 left-2 p-1.5 bg-black/50 backdrop-blur-md rounded-lg shadow-sm">
                      {isInsta ? (
          ...
          return (
              <div key={uniqueKey} onClick={() => setCollectionMode({ isOpen: true, title: item.title, relatedDeals: item.deals })} className={`break-inside-avoid rounded-2xl p-2.5 shadow-sm cursor-pointer group hover:border-emerald-300 transition-colors border ${currentTheme.card}`}>
                  <div className="grid grid-cols-2 gap-1.5 mb-2 relative">
                      ...
                      {item.deals.length > 4 && (
                          <div className="absolute bottom-0 right-0 w-[48%] aspect-square bg-black/80 backdrop-blur-md rounded-lg flex flex-col items-center justify-center leading-none shadow-inner border border-white/10">
                              <span className="text-white font-black text-sm">+{item.deals.length - 3}</span>
                          </div>
                      )}
                  </div>
                  <span className="bg-blue-500/20 text-blue-300 text-[8px] font-black px-1.5 py-0.5 rounded flex w-fit items-center gap-1 border border-blue-500/30 uppercase">
                      <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>
                      Collection
                  </span>
  ...
  if (isLoading) return <div className="min-h-screen bg-slate-900 flex items-center justify-center"><div className="w-10 h-10 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div></div>;
  ...
  return (
    <div className="min-h-screen bg-black/90 flex justify-center font-sans selection:bg-emerald-500 selection:text-white">
      ...
      <style jsx>{`
        @keyframes sequenceBounce {
          0%, 80%, 100% { transform: translateY(0); }
          40% { transform: translateY(-8px); }
        }
        .social-sequence-bounce { animation: sequenceBounce 2.5s infinite ease-in-out; }
      `}</style>

      {/* 🚨 NAYA: Fake Sales Social Proof Popup (Bottom Centered & Professional English) */}
      <div className="fixed bottom-6 w-full max-w-md z-[90] pointer-events-none px-4 flex justify-center">
          {liveSale && (
              <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700 shadow-[0_10px_40px_rgba(0,0,0,0.5)] rounded-2xl p-3 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-5 duration-500 max-w-[95%] pointer-events-auto">
                  ...
                  <p className="text-[11.5px] font-medium text-slate-200 leading-tight">
                      A follower of <span className="font-extrabold text-white">{creator.username || creator.name}</span> bought an item worth <span className="font-black text-emerald-400">₹{liveSale.amount}</span> on <span className="font-bold text-white">{liveSale.store}</span> {liveSale.time}! 🎉
                  </p>
              </div>
          )}
      </div>
      ...
      {toastMessage && (
          <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-md text-white text-[11px] font-black px-6 py-2.5 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-[200] animate-in fade-in slide-in-from-bottom-5 border border-white/20">
              ✅ {toastMessage}
          </div>
      )}
      ...
      {/* 🏠 ADD TO HOME DRAWER */}
      {isHomeDrawerOpen && (
        <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/60 backdrop-blur-sm px-4 pb-4">
            <div className="w-full max-w-md bg-white rounded-[2.5rem] p-8 animate-in slide-in-from-bottom duration-300 text-slate-900 shadow-2xl">
                ...
                <div className="space-y-4 text-sm font-medium text-slate-600 leading-relaxed">
                    <p className="bg-blue-50 p-3 rounded-xl border border-blue-100"><span className="font-bold text-blue-700">For Android:</span><br/> Tap the <span className="font-bold">three dots (⋮)</span> in Chrome and select <span className="font-bold text-slate-900">"Add to Home screen"</span>.</p>
                    <p className="bg-emerald-50 p-3 rounded-xl border border-emerald-100"><span className="font-bold text-emerald-700">For iOS (iPhone):</span><br/> Tap the <span className="font-bold text-slate-900">Share icon (📤)</span> at the bottom of Safari and select <span className="font-bold text-slate-900">"Add to Home Screen"</span>.</p>
                </div>
                <button onClick={() => setIsHomeDrawerOpen(false)} className="w-full bg-slate-900 text-white font-bold py-4 rounded-2xl mt-8 shadow-xl hover:bg-slate-800 transition-colors">Got it!</button>
            </div>
        </div>
      )}
      ...
      {theatreMode.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/95 backdrop-blur-md">
            <div className="w-full max-w-md relative h-[100dvh] overflow-y-auto overflow-x-hidden scroll-smooth [&::-webkit-scrollbar]:hidden">
                ...
                <div className="sticky top-0 h-[100dvh] w-full flex items-center justify-center z-0 bg-black pointer-events-auto">
      ...
      {/* 🚨 FIX: justify-center ke saath items-end add kiya taaki desktop par bhi sahi se center bottom mein dikhe */}
      {(collectionMode.isOpen || categoryDrawer.isOpen) && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-sm">
            <div className={`w-full max-w-md ${currentTheme.bg} ${currentTheme.text} rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.3)] max-h-[85vh] flex flex-col animate-in slide-in-from-bottom border-t border-white/10`}>
      ...
      {/* ----------------- PHASE 1: LOCKED HEADER AREA ----------------- */}
      <div className="p-3 pb-0 relative z-20 overflow-hidden">
          <div className="relative w-full aspect-[4/1] mb-4 group">
              <button onClick={() => setIsHomeDrawerOpen(true)} className="absolute top-2 right-2 z-10 bg-black/60 hover:bg-black/80 text-white rounded-full flex items-center gap-1.5 px-3 py-1.5 backdrop-blur-md shadow-md transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                  <span className="text-[9px] font-black uppercase tracking-tight">Add to Home</span>
              </button>
          ...
          <div className="flex items-center gap-3 relative z-10">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-white/10 rounded-full flex-shrink-0 border-2 border-emerald-500 p-[2px] shadow-xl backdrop-blur-sm self-start">
              ...
              <div className="flex-1 min-w-0">
                  <h1 className="text-lg md:text-xl font-black truncate drop-shadow-sm mb-0.5">{creator.name}</h1>
                  <p className="text-[11px] font-semibold opacity-90 line-clamp-3 leading-snug drop-shadow-sm pr-1">{creator.bio || "Welcome to my official storefront!"}</p>
              </div>
              <div className="flex flex-col gap-2 justify-center flex-shrink-0">
                  <button onClick={() => { if(navigator.share) navigator.share({url: window.location.href}); else alert('Link Copied'); }} className="w-8 h-8 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center border border-white/20 backdrop-blur shadow-sm"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg></button>
                  <button onClick={() => alert("Subscribe feature coming soon!")} className="w-8 h-8 bg-emerald-500 text-white rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.5)] hover:scale-105 transition-transform"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg></button>
              </div>
          </div>

          {creator.socialHandles?.length > 0 && (
              <div className="flex justify-center gap-4 mt-4 mb-1 overflow-visible">
                  {creator.socialHandles.map((social, index) => {
                      const details = getSocialDetails(social);
                      return (
                          <a key={index} href={details.url} target="_blank" rel="noopener noreferrer" style={{ animationDelay: `${index * 0.15}s` }} className="social-sequence-bounce flex flex-col items-center group cursor-pointer">
                              <div className="w-10 h-10 bg-white shadow-md rounded-full flex items-center justify-center border border-white/50 group-hover:scale-110 transition-transform">{details.icon}</div>
      ...
      {/* ----------------- PHASE 2: 4 PREMIUM TABS & FEEDS ----------------- */}
      
      <div className={`px-4 sticky top-0 z-40 pt-4 pb-3 ${currentTheme.tabBg} border-b backdrop-blur-md`}>
        <div className="flex overflow-x-auto snap-x bg-black/10 border border-white/10 rounded-full p-1 gap-1 [&::-webkit-scrollbar]:hidden shadow-inner">
          <button onClick={() => setActiveTab("home")} className={`flex items-center justify-center gap-1.5 flex-shrink-0 px-5 py-2.5 text-[11px] font-black rounded-full transition-all snap-start ${activeTab === "home" ? currentTheme.tab + ' shadow-md' : 'opacity-60 hover:opacity-100'}`}>
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
            SHOP ALL
          </button>
          
          <button onClick={() => setActiveTab("trending")} className={`flex items-center justify-center gap-1 flex-shrink-0 px-5 py-2.5 text-[11px] font-black rounded-full transition-all snap-start ${activeTab === "trending" ? currentTheme.tab + ' shadow-md' : 'opacity-60 hover:opacity-100'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z"></path><path d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z"></path></svg>
            TRENDING
          </button>
          
          <button onClick={() => setActiveTab("telegram")} className={`flex items-center justify-center gap-1.5 flex-shrink-0 px-5 py-2.5 text-[11px] font-black rounded-full transition-all snap-start relative ${activeTab === "telegram" ? currentTheme.tab + ' shadow-md' : 'opacity-60 hover:opacity-100'}`}>
          ...
          <button onClick={() => setActiveTab("categories")} className={`flex items-center justify-center gap-1.5 flex-shrink-0 px-5 py-2.5 text-[11px] font-black rounded-full transition-all snap-start ${activeTab === "categories" ? currentTheme.tab + ' shadow-md' : 'opacity-60 hover:opacity-100'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg>
            CATEGORIES
          </button>
        </div>
      </div>
                        ...
                        <div key={idx} className="space-y-3">
                            <div className="flex justify-between items-center px-1">
                                ...
                                {categoryGroups[catName].length > 4 && (
                                    <button onClick={() => setCategoryDrawer({ isOpen: true, title: catName, relatedDeals: categoryGroups[catName] })} className="text-[10px] font-bold opacity-70 hover:opacity-100 flex items-center gap-1 bg-white/5 px-2 py-1 rounded border border-white/10">See All ➔</button>
                                )}
                            </div>
...
function GridProductCard({ deal, onClick, themeCardClass, onToast }) {
    return (
        <div className={`border shadow-sm rounded-2xl p-2 flex flex-col hover:scale-[1.02] hover:border-emerald-400 transition-all cursor-pointer group backdrop-blur-sm ${themeCardClass}`} onClick={onClick}>
            
            <div className="w-full aspect-square bg-white rounded-xl mb-2 relative p-1 overflow-hidden shadow-inner border border-white/50">
                ...
                <span className="absolute bottom-0 left-0 bg-slate-900/80 backdrop-blur-sm text-white text-[9px] font-black px-2.5 py-1 rounded-tr-xl z-10">
                    {deal.store || "Exclusive"}
                </span>
            </div>
            ...
            {deal.saleEndTime && (
                <div className="px-1">
                    <LiveTimer targetDate={deal.saleEndTime} />
                </div>
            )}

            <div className="flex items-center gap-2 mt-auto pt-2 border-t border-white/10 px-1">
                {deal.price ? (
                    <>
                        <span className="text-sm font-black drop-shadow-sm">{deal.price}</span>
                        <button className="flex-1 bg-emerald-500 text-white text-[11px] font-bold py-1.5 rounded-lg shadow-[0_4px_10px_rgba(16,185,129,0.3)] hover:bg-emerald-600 transition-colors text-center">
                            Shop Now
                        </button>
                    </>
                ) : (
                    <button className="w-full bg-emerald-500 text-white text-[11px] font-bold py-1.5 rounded-lg shadow-[0_4px_10px_rgba(16,185,129,0.3)] hover:bg-emerald-600 transition-colors text-center">
                        Shop Now
                    </button>
                )}
            </div>
