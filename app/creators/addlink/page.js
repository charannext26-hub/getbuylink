              ...
              {successDrawer.links.length > 0 ? (
                successDrawer.links.map((linkObj, idx) => (
                  <div key={idx} className="bg-white border border-slate-200 shadow-sm p-4 rounded-xl flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                   ...
                   {/* 👇 NAYA: Split Buttons (65% Share, 35% Auto-DM with Text) */}
                    <div className="flex w-full md:w-auto gap-2 flex-shrink-0 mt-3 md:mt-0">
                      
                      {/* Share Button (65% Width) */}
                      <button onClick={() => handleShare(linkObj.title, linkObj.url)} className="w-[65%] px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-lg shadow-md transition-colors flex items-center justify-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg>
                        Share
                      </button>
                      
                      {/* Auto-DM Button (35% Width with Icon & Text) */}
                      <button onClick={() => { setAutoDmModal({ show: true, linkObj }); setDmTab('guide'); setPlayVideo(false); }} className="w-[35%] px-2 py-2.5 bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] hover:opacity-90 text-white font-bold text-sm rounded-lg shadow-md transition-transform active:scale-95 flex items-center justify-center gap-1.5 whitespace-nowrap">
      ...
      >
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-6">
           <div className="space-y-4">
             ...
             <input type="text" placeholder="Paste insta,youtube vedio link..." value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl p-3.5 text-slate-900 font-bold text-sm focus:border-blue-500 outline-none" />
           </div>
        ...
        {previewDeals.length > 0 && !isFetching && (
          <div className="pt-4">
            ...
            >
              {isPublishing ? "⏳ PUBLISHING TO PAGE..." : "🚀 PUBLISH TO PAGE"}
            </button>
          </div>
        )}
