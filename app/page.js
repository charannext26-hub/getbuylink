...
import { useSession, SessionProvider } from "next-auth/react"; // SessionProvider import kiya
import { useRouter } from "next/navigation";

// 1. Asli Content ka Component
function LandingContent() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [username, setUsername] = useState("");

  // Agar user logged in hai, toh automatically /creators par bhej do
  useEffect(() => {
  ...
  // Jab tak check ho raha hai, ek smooth loading spinner dikhao
  ...
  // Agar logged in nahi hai, toh Landing Page dikhao
  return (
    <div className="min-h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      <nav className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-2">
          {/* Aapka FavyLink Graphic Icon */}
          <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-black text-xs tracking-tighter shadow-md">
            FL
          </div>
          <span className="font-extrabold text-xl tracking-tight text-slate-900">FavyLink</span>
        </div>
        <div className="flex items-center gap-4">
          <Link href="/login" className="text-sm font-bold text-slate-600 hover:text-slate-900 transition-colors hidden sm:block">
            Log in
          </Link>
          <Link href="/login" className="bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold px-5 py-2.5 rounded-xl transition-all shadow-md shadow-slate-900/20">
            Sign Up Free
          </Link>
        </div>
      </nav>
